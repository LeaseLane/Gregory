...
**P2 : inventaire et procédure faits** — voir `docs/SECRETS.md`.
L'inventaire compte **10 secrets réels** côté fonctions edge et 6 côté
GitHub. Le mandat en annonçait 11 : l'écart vient de `SUPABASE_ANON_KEY`,
publique par conception.

Reste la rotation elle-même (P2.2), la séparation préproduction (P2.3,
bloquée par P1) et le recensement des admins du projet Supabase (P2.4).
Ce dernier est le préalable aux deux autres — tourner un secret que
d'anciens collaborateurs connaissent encore ne protège de rien.
